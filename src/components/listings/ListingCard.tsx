...
export default function ListingCard({ listing }: ListingCardProps) {
  ...
  return (
    <Link
      ...
      className="block bg-white rounded-2xl border border-slate-200 hover:border-slate-300 hover:shadow-lg transition-all duration-300 overflow-hidden group"
    >
      {/* Image */}
      <div className="aspect-[4/3] bg-slate-100 relative overflow-hidden">
        ...
        {isSold && (
          <div className="absolute inset-0 bg-slate-900/40 flex items-center justify-center">
            <Badge variant="sold" className="text-sm px-3 py-1">Vendu</Badge>
          </div>
        )}
      ...
      {/* Content */}
      <div className="p-4">
        <h3 className="font-medium text-slate-900 text-sm line-clamp-2 mb-2 leading-snug group-hover:text-brand-600 transition-colors">
          {listing.title}
        </h3>
        <p className={`font-bold text-lg mb-3 ${isSold ? 'text-slate-400 line-through' : 'text-brand-600'}`}>
